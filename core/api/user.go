...
func listUser(c *gin.Context) {
	...
	group := c.Query("group")
	if name != "" {
		users, err := app.App.DBIo.DescribeUser(name)
		if err != nil {
	...
	if group != "" {
		users, err := app.App.DBIo.QueryUserByGroup(group)
		if err != nil {
			c.JSON(500, err.Error())
			return
		}
	...
	// 否则查询所有
	users, err := app.App.DBIo.QueryAllUser()
	if err != nil {
		c.JSON(500, err.Error())
		return
	}
...
func addUser(c *gin.Context) {
	...
	_, err := app.App.DBIo.CreateUser(&req)
	if err != nil {
		c.JSON(500, err.Error())
		return
	}
...
func updateUserGroup(c *gin.Context) {
	...
	if err := app.App.DBIo.PatchUserGroup(id, req); err != nil {
		c.JSON(500, err.Error())
		return
	}
	c.String(200, "success")
...
func updateUser(c *gin.Context) {
	...
	if err := app.App.DBIo.UpdateUser(id, *req); err != nil {
		c.JSON(500, err.Error())
		return
	}
	c.String(200, "success")
